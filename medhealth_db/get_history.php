...
while ($row_order = mysqli_fetch_array($query_select_order)) {
    ...
    $query_select_order_detail = mysqli_query($connection, "SELECT order_details.*, product.name FROM order_details JOIN product on order_details.id_product = product.id_product WHERE order_details.invoice = '$noInvoice'");
